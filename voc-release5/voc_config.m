...
% things to tweak: BASE_DIR, onCluster, depth
...
BASE_DIR    = '/home/eecs/edmundye/psiCode';
...
%
  ...
  % Clobber with overrides passedmodel = pascal_train_root_only(cls, n, note); in as arguments
...
% onCluster
conf = cv(conf, 'onCluster', onCluster);

% Depth
conf = cv(conf, 'depth', depth);

% only root
conf = cv(conf, 'root_only', root_only);
...
%conf = cv(conf, 'pascal.dev_kit'model = pascal_train_root_only(cls, n, note);, [conf.paths.base_dir '/VOC' ...
%                                   conf.pascal.year '/VOCdevkit/']);
% For INRIA person                                   
%conf = cv(conf, 'pascal.dev_kit', [conf.paths.base_dir '/INRIA/VOCdevkit/']);
%conf = cv(conf, 'pascal.dev_kit', [conf.paths.base_dir '/NYU_PASCAL/VOCdevkit/']);
conf = cv(conf, 'pascal.dev_kit', [c.base '/VOCdevkit/']);
...
%                                     conf.project '/' conf.pascal.year '/']); % changed!

conf = cv(conf, 'paths.model_dir', [c.cached '/']);
...
if depth
    conf = cv(conf, 'features.dim', 50);
    conf = cv(conf, 'features.truncation_dim', 50);
else
