...
for i = 1:interval
  ...
  if extra_interval > 0
    ...
    pyra.feat{i} = features(scaled, sbin/4); %replace
    pyra.scales(i) = 4/sc^(i-1);
  end
  % (sbin/2) x (sbin/2) features
  pyra.feat{i+extra_interval} = features(scaled, sbin/2); %replace
  pyra.scales(i+extra_interval) = 2/sc^(i-1);
  % sbin x sbin HOG features
  pyra.feat{i+extra_interval+interval} = features(scaled, sbin); %replace
  ...
  for j = i+interval:interval:max_scale
    ...
    pyra.feat{j+extra_interval+interval} = features(scaled, sbin); %replace
