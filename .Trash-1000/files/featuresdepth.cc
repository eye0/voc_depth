...
mxArray *process(const mxArray *mximage, const mxArray *mxsbin) {
  ...
  if (mxGetNumberOfDimensions(mximage) != 1 ||
      dims[2] != 3 ||
...
// each pixel casts a weighted vote to adjacent cells(tent kernel)
      if (ixp >= 0 && iyp >= 0) {
...
// .5 something to do with pca
      ...
      src = hist + (x+1)*blocks[0] + (y+1);
      for (int o = 0; o < 9; o++) {
// index in the opposite direction
